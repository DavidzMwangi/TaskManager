        ...
        <div v-if="($page.props.flash.error || Object.keys($page.props.errors).length > 0) && show"
            ...
            <div class="flex items-center">
                ...
                <div v-if="$page.props.flash.error" class="py-4 text-white text-sm font-medium">
                    <template v-if="Array.isArray($page.props.flash.error)">
                        <ul>
                            <li v-for="(err, index) in $page.props.flash.error" :key="index">{{ err }}</li>
                        </ul>
                    </template>
                    <template v-else-if="typeof $page.props.flash.error === 'object'">
                        <!-- If the error is an object -->
                        <ul>
                            <li v-for="(messages, field) in $page.props.flash.error" :key="field">
                                <strong>{{ field }}:</strong>
                                <ul>
